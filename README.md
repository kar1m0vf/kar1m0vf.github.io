...
- The midpoint between Nar and Trendyol has a reversible native-scroll passage: the camera centers, enters a thicker glass helix and approaches a light that opens into the price scene. The entrance flares behind the viewer; the distant coils converge along the light's axis. A soft blue glow bridges the hidden camera reset. A direct next-project link remains available during the passage. Reduced motion and unavailable WebGL use normal flow. Keyboard-accessible About tabs remain available.
...
- The original Mini Blaster remains playable in place, with keyboard and touch controls and a locally saved best score. On desktop its field uses roughly half the content width; mobile uses the available width. The canvas shares the playfield's rounded clipping during gameplay. A perfect run earns 50,000: eleven drones worth 2,000 each and a boss worth 28,000, including per-hit rewards. Scores use a versioned storage key; a previous perfect 2,630 record converts to 50,000. There is no fullscreen/expand mode. Escape pauses the game.
...
- The loading filament hands over to a small glass loop in the existing Hero canvas, which unfolds into the knot. Open thread poses continue beyond the frame. Glass is 35% thicker throughout; circular sections follow the blended centerline, with transported normals and local bend smoothing. The middle retains its triangle budget, redistributing vertices from the core and hidden tunnel entrance to the visible glass. Idle geometry buffers stay untouched; render resolution and effect passes are unchanged.
